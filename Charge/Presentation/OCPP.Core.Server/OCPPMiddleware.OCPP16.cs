...
namespace OCPP.Core.Server
{
    public partial class OCPPMiddleware
    {
        ...
        private async Task RemoteStartTransaction(ChargePointStatus chargePointStatus, HttpContext apiCallerContext)
        {
            ILogger logger = _logFactory.CreateLogger("OCPPMiddleware.OCPP16");
            ControllerOCPP16 controller16 = new ControllerOCPP16(_configuration, _logFactory, chargePointStatus);
         
            string jsonContent = string.Empty;
            using (StreamReader reader = new StreamReader(apiCallerContext.Request.Body, Encoding.UTF8))
            {
            ...
            msgOut.UniqueId = Guid.NewGuid().ToString("N");
            msgOut.JsonPayload = jsonContent;
            msgOut.TaskCompletionSource = new TaskCompletionSource<string>();
        ...
        private async Task RemoteStopTransaction(ChargePointStatus chargePointStatus, HttpContext apiCallerContext)
        {
            ILogger logger = _logFactory.CreateLogger("OCPPMiddleware.OCPP16");
            ControllerOCPP16 controller16 = new ControllerOCPP16(_configuration, _logFactory, chargePointStatus);
