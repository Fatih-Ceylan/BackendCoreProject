    ...
    public partial class OCPPMiddleware
    {
        ...
        public async Task Invoke(HttpContext context)
        {
            ...
            if (context.Request.Path.StartsWithSegments("/OCPP"))
            {
                ...
                {
                    routeName = parts[2];
                    chargepointIdentifier = parts[3];
                    context.Request.Headers.Add("Route-Name", routeName);
                }
                else
                {
                    _logger.LogWarning("OCPPMiddleware => Invalid path format. Expected format is /OCPP/RouteName/ChargepointId");
                    context.Response.StatusCode = (int)HttpStatusCode.BadRequest;
                    return;
                }

                _logger.LogInformation("OCPPMiddleware => Connection request with chargepoint identifier = '{0}', database = '{1}'", chargepointIdentifier, routeName);

                // Known chargepoint?
                {
                    DbContextOptions<GChargeDbContext> options = new();
                    IHttpContextAccessor httpContextAccessor = new HttpContextAccessor();
                    using (GChargeDbContext dbContext = new GChargeDbContext(options, _configuration, httpContextAccessor))
                    {
                    ...
                    else if (cmd == "RemoteStartTransaction")
                    {
                        if (!string.IsNullOrEmpty(urlChargePointId))
                        {
                            try
                            {
                                ...
                                if (_chargePointStatusDict.TryGetValue(urlChargePointId, out status))
                                {
                                    await RemoteStartTransaction(status, context);
                                }
                                else
                                {
                    ...
                    else if (cmd == "RemoteStopTransaction")
                    {
                        if (!string.IsNullOrEmpty(urlChargePointId))
                        {
                            try
                            {
                                ...
                                if (_chargePointStatusDict.TryGetValue(urlChargePointId, out status))
                                {
                                    await RemoteStopTransaction(status, context);
                                }
                                else
                                {
            ...
            else if (context.Request.Path.StartsWithSegments("/"))
            {
                try
                {
                    ...
                    if (showIndexInfo)
                    {
                        ...
                        foreach (string key in _chargePointStatusDict.Keys)
                        {
